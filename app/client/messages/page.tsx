...
type Conversation = {
  ...
  freelancer?: { first_name?: string | null; last_name?: string | null; personal_img_url: string | null; freelancer_id?: number | null } | null;
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...
        message: "Offer from client via chat",
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider pl-1 pt-2">Total Price</label>
                    <input
                      value={totalPrice}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="relative">
                              <input
                                ...
                                disabled={locked}
                              />
                              <div className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs font-bold">Days</div>
                            </div>
                          </div>
                          {milestones.length > 1 && (
...
function ClientOfferViewerModal({
  ...
  const actorRole = "client" as const;
  ...
  const confirmContract = async () => {
    ...
    const res = await fetch(`/api/proposals/${proposal.proposal_id}/respond`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "confirm", actor: "client" }),
    });
    const json = await res.json();
    if (!res.ok) {
    ...
    if (conversationId && currentUserId) {
      await supabase.from("messages").insert({
        ...
        sender_auth_id: currentUserId,
        sender_role: "client",
        body: `Client confirmed the contract (Proposal #${proposal.proposal_id}).`,
      });
      await supabase.from("conversations").update({ last_message_at: new Date().toISOString() }).eq("id", conversationId);
    }
      ...
      {open && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <motion.div
            initial={
              clickPosition
                ? { opacity: 0, scale: 0.1, x: clickPosition.x - safeW / 2, y: clickPosition.y - safeH / 2 }
...
export default function ClientMessagesPage() {
  ...
  const computeUnreadForClient = useCallback(async (c: Conversation) => {
    const lastSeen = c.client_last_seen_at || EPOCH_ISO;

    const { count, error } = await supabase
      .from("messages")
      .select("id", { count: "exact", head: true })
      .eq("conversation_id", c.id)
      .in("sender_role", ["freelancer", "admin"])
  ...
  const markConversationSeen = useCallback(async (conversationId: string) => {
    ...
    const { error } = await supabase.rpc("mark_conversation_seen_as_client", {
      p_conversation_id: conversationId,
    });

    if (error) {
      console.error("[chat] mark_conversation_seen_as_client failed:", error);
      return;
    }

    setConversations((prev) =>
      prev.map((c) => (c.id === conversationId ? { ...c, client_last_seen_at: nowIso, unread_count: 0 } : c))
    );
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const freelancerIds = Array.from(new Set(rawConvs.map((c) => c.freelancer_id).filter(Boolean)));
      const { data: freelancers, error: freelancersError } = await supabase
        .from("freelancers")
        .select("freelancer_id, first_name, last_name, personal_img_url")
        .in("freelancer_id", freelancerIds);

      if (freelancersError) console.error("Error fetching freelancers:", freelancersError);

      const enriched = await Promise.all(
        rawConvs.map(async (c) => {
          const job = jobs?.find((j) => String(j.job_post_id) === String(c.job_post_id));
          const freelancer = freelancers?.find((f) => String(f.freelancer_id) === String(c.freelancer_id));
          ...
          const unread_count = await computeUnreadForClient(c as Conversation);

          return {
            ...(c as Conversation),
            job_posts: job ? { title: job.title } : undefined,
            freelancer: (freelancer as any) || undefined,
          ...
          (payload) => {
            ...
            setConversations((prev) =>
              prev.map((c) => {
                ...
                const fromOtherSide = newMsg.sender_role !== "client";
            ...
            if (newMsg.sender_role !== "client" && document.visibilityState === "visible") {
              void markConversationSeen(selectedId);
            }
          }
        )
  ...
  // Avoid crashing on invalid remote images:
  ...
  return (
    <div className="flex h-screen bg-[#fbfbfd]">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-200 border border-gray-100 overflow-hidden flex items-center justify-center text-sm font-semibold text-gray-500">
                  {selectedConv?.freelancer?.personal_img_url && headerImgOk ? (
                    <img
                      src={selectedConv.freelancer.personal_img_url}
                      alt="Freelancer"
                      className="w-full h-full object-cover"
                      onError={() => setHeaderImgOk(false)}
                    />
                  ) : (
                    initials(displayFreelancerName(selectedConv?.freelancer))
                  )}
                </div>
                <div>
                  <h2 className="font-bold text-gray-900 text-[15px] leading-tight">{displayFreelancerName(selectedConv?.freelancer)}</h2>
      ...
      <OfferInlineModal
        ...
        clientId={myClientId}
        freelancerId={selectedConv?.freelancer_id || null}
